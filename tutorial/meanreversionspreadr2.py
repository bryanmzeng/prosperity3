...
class Trader:
    def __init__(self):
        # Strategy parameters (calibrated from historical data)
        self.params = {
            Product.SPREAD: {
                "default_spread_mean": 48.7624,  # Fixed mean based on historical analysis
                "default_spread_std": 83.5354,   # Initial standard deviation
                "spread_std_window": 50,         # Rolling window size for std dev calculation
                "zscore_threshold_upper": 2.0,   # Z-score threshold for selling the spread
                "zscore_threshold_lower": -2.0,  # Z-score threshold for buying the spread
                "exit_buffer": 0.5,              # Buffer for exiting positions
                "target_position": 58,           # Target position size
            }
        }
    ...
    def get_synthetic_basket_order_depth(
        ...
        # Constants
        CROISSANTS_PER_BASKET = self.basket_weights[Product.CROISSANTS]
        JAMS_PER_BASKET = self.basket_weights[Product.JAMS]
        DJEMBES_PER_BASKET = self.basket_weights[Product.DJEMBES]
    ...
    def calculate_spread(self, order_depths: Dict[str, OrderDepth]) -> Optional[float]:
        """
        Calculate the spread between the basket and its components
        Returns the spread value or None if it can't be calculated
        """
        ...
        # Spread = Basket Price - Synthetic Basket Price
        spread = basket_swmid - synthetic_swmid
        return spread
        
    def calculate_zscore(self, spread: float, spread_history: List[float], mean_spread: float) -> float:
        """
        Calculate z-score using the fixed mean and rolling standard deviation
        """
        # If we don't have enough spread history, use the default standard deviation
        if len(spread_history) < self.params[Product.SPREAD]["spread_std_window"]:
            std_dev = self.params[Product.SPREAD]["default_spread_std"]
        else:
            # Calculate standard deviation from the most recent window
            window_data = spread_history[-self.params[Product.SPREAD]["spread_std_window"]:]
        ...
        # Calculate z-score: (spread - mean) / std_dev
    ...
    def convert_synthetic_basket_orders(
        ...
        for order in synthetic_orders:
            ...
            croissants_order = Order(
                Product.CROISSANTS,
                croissants_price,
                quantity * self.basket_weights[Product.CROISSANTS],
            )
            jams_order = Order(
                Product.JAMS,
                jams_price,
                quantity * self.basket_weights[Product.JAMS],
            )
            djembes_order = Order(
                Product.DJEMBES, djembes_price, quantity * self.basket_weights[Product.DJEMBES]
            )
    ...
    def calculate_effective_position(self, positions: Dict[str, int]) -> int:
        ...
        croissants_equivalent = positions.get(Product.CROISSANTS, 0) / self.basket_weights[Product.CROISSANTS]
        jams_equivalent = positions.get(Product.JAMS, 0) / self.basket_weights[Product.JAMS]
        djembes_equivalent = positions.get(Product.DJEMBES, 0) / self.basket_weights[Product.DJEMBES]
        ...
        if buy_basket:
            ...
            if basket_depth.sell_orders:
                ...
                if order_depths[Product.CROISSANTS].buy_orders:
                    ...
                    result[Product.CROISSANTS].append(
                        Order(Product.CROISSANTS, croissants_best_bid, -quantity * self.basket_weights[Product.CROISSANTS])
                    )
                
                if order_depths[Product.JAMS].buy_orders:
                    jams_best_bid = max(order_depths[Product.JAMS].buy_orders.keys())
                    result[Product.JAMS].append(
                        Order(Product.JAMS, jams_best_bid, -quantity * self.basket_weights[Product.JAMS])
                    )
                
                if order_depths[Product.DJEMBES].buy_orders:
                    djembes_best_bid = max(order_depths[Product.DJEMBES].buy_orders.keys())
                    result[Product.DJEMBES].append(
                        Order(Product.DJEMBES, djembes_best_bid, -quantity * self.basket_weights[Product.DJEMBES])
                    )
        else:
            # Sell basket
            if basket_depth.buy_orders:
                ...
                if order_depths[Product.CROISSANTS].sell_orders:
                    ...
                    result[Product.CROISSANTS].append(
                        Order(Product.CROISSANTS, croissants_best_ask, quantity * self.basket_weights[Product.CROISSANTS])
                    )
                
                if order_depths[Product.JAMS].sell_orders:
                    jams_best_ask = min(order_depths[Product.JAMS].sell_orders.keys())
                    result[Product.JAMS].append(
                        Order(Product.JAMS, jams_best_ask, quantity * self.basket_weights[Product.JAMS])
                    )
                
                if order_depths[Product.DJEMBES].sell_orders:
                    djembes_best_ask = min(order_depths[Product.DJEMBES].sell_orders.keys())
                    result[Product.DJEMBES].append(
                        Order(Product.DJEMBES, djembes_best_ask, quantity * self.basket_weights[Product.DJEMBES])
                    )
